...
unsigned int* padding(string str) {
	...
	unsigned int *StrBytes = new unsigned int[num * 16] {};//n*16*4*8 = 512 * n bits
	...
	for (unsigned int i = 0; i <str.length(); i++) //共用 str.length()/4 个 unsigned int位被占用
    {
		StrBytes[i / 4] |= (str[i]) << ((i % 4) * 8);
	}

	//尾部添加1 一个unsigned int保存4个字符信息,所以用128左移
	StrBytes[str.length() / 4] |= 0x80 << (((str.length() % 4)) * 8);

	/*
	*添加原长度，长度指位的长度，所以要乘8，然后是小端序，所以放在倒数第二个,这里长度只用了32位
	*/
	StrBytes[num * 16 - 2] = str.length() * 8;
	return StrBytes;
